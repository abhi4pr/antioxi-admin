...
const Rewards = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        const response = await api.get(`${API_URL}/rewards?page=${currentPage}&perPage=${perPage}`);
      ...
      buttons: [
        {
          ...
          onClick: async () => {
            try {
              await api.delete(`${API_URL}/rewards/${rewardId}`);
      ...
      <Row className="justify-content-center">
        <Card title="Rewards" isOption>
          <div className="d-flex justify-content-between align-items-center mb-4 mt-4">
            ...
            <Button variant="primary" as={Link} to="/reward" className="btn-sm">
              Add New
            </Button>
          </div>
          <input
