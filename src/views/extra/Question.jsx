...
const Question = ({ questionToEdit }) => {
  ...
  const validateForm = () => {
    ...
    if (!formData.sequence.trim()) {
      newErrors.sequence = 'Question sequence is required';
    }
          ...
          {formData.options.map((option, index) => (
            <Form.Group as={Row} className="mb-3" key={index} controlId={`formOption${index}`}>
              ...
              <Col sm={10}>
                ...
                />
                <Form.Control.Feedback type="invalid">{errors[`option${index}`]}</Form.Control.Feedback>
              </Col>
            </Form.Group>
          ))}

          <Form.Group as={Row} className="mb-3" controlId="formSequence">
            <Form.Label column sm={2} style={{ textAlign: 'right' }}>
              Sequence:
            </Form.Label>
            <Col sm={10}>
              <Form.Select name="sequence" value={formData.sequence} onChange={handleChange} isInvalid={!!errors.sequence}>
